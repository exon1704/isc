...
import {ContactoComponent} from "@isc/modulo/generales/contacto/contacto.component";
...
export class RegistrarTicketComponent implements OnInit, OnDestroy {
  ...
  generales: Generales = {
    unidad: undefined, info: undefined
  };
    ...
    if (this.formReporte.get('unidad').value) {
      this.unidadService.obtenerGenerales(this.formReporte.get('unidad').value.id).subscribe({
        next: t => {
          if (t.code == 200) {
            this.generales = {
              unidad: this.formReporte.get('unidad').value, info: t.data
            }
